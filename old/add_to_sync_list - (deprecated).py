...
API_BASE_URL = "http://localhost:6767"
...
def blacklist_subtitle(is_movie):
    if is_movie:
        url = f"{API_BASE_URL}/api/movies/blacklist?radarrid={episode_id}"
    else:
        url = f"{API_BASE_URL}/api/episodes/blacklist?seriesid={series_id}&episodeid={episode_id}"
...
def download_new_subtitle(is_movie):
    if is_movie:
        url = f"{API_BASE_URL}/api/movies/subtitles?radarrid={episode_id}"
    else:
        url = f"{API_BASE_URL}/api/episodes/subtitles?seriesid={series_id}&episodeid={episode_id}"
